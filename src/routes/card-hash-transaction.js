...
function readQR (bot, reply) {
  ...
  bot.getFile(reply.photo[reply.photo.length-1].file_id)
    .then((file) => {
      const photoURL = 'https://api.telegram.org/file/bot'+keys.BOT+'/'+file.file_path
      return axios
    ...
    .then(res => {
      ...
      if (res.data[0].symbol[0].data === null) {bot.sendMessage(reply.chat.id, 'Não entendir');return}
